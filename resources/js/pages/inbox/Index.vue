...
type Entry = {
    kind: 'assigned' | 'created' | 'commented' | 'project_update';
    occurred_at: string;
    issue: FeedIssue;
    actor: Actor | null;
    snippet: string | null;
...
const grouped = computed(() => {
    ...
    for (const e of props.feed) {
        ...
        if (ageDays < 1) today.push(e);
        else if (ageDays < 2) yesterday.push(e);
        else earlier.push(e);
    }
    return [
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col overflow-hidden">
        ...
        >
            ...
            <span class="text-[12px] text-muted-foreground">{{ counts.total }}</span>
        </header>
        ...
        >
            <div class="max-w-sm">
                ...
                <h2 class="mt-4 text-base font-medium">You're all caught up</h2>
                <p class="mt-2 text-sm text-muted-foreground">
                    Notifications about issues you're assigned to, created or
                    commented on will appear here.
                </p>
            </div>
        </div>
        ...
        <div v-else class="flex-1 overflow-y-auto">
            <section v-for="group in grouped" :key="group.label">
                <div
                    class="sticky top-0 z-10 bg-muted/40 px-4 py-1.5 text-[11px] font-medium uppercase tracking-wide text-muted-foreground backdrop-blur"
                >
                    {{ group.label }}
                </div>
                <ul class="divide-y divide-border">
                    <li v-for="(entry, idx) in group.entries" :key="`${entry.kind}-${entry.issue.id}-${idx}`">
                        <Link
                        ...
                        >
                            ...
                            >
                                <component :is="actionIcon(entry.kind)" class="size-3" />
                            </span>

                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2 text-[13px]">
                                    <span class="font-medium text-foreground">{{ entry.actor?.name ?? 'System' }}</span>
                                    <span class="text-muted-foreground">{{ actionLabel(entry) }}</span>
                                    <span class="text-muted-foreground">·</span>
                                    <span class="text-muted-foreground">{{ relativeTime(entry.occurred_at) }}</span>
                                </div>

                                <div class="mt-1 flex min-w-0 items-center gap-2 text-[12.5px]">
                                    <span
                                        v-if="entry.issue.team"
                                        class="flex size-4 shrink-0 items-center justify-center rounded text-[9px] font-semibold text-white"
                                        :style="{
                                            backgroundColor:
                                                entry.issue.team.color || '#6366f1',
                                        }"
                                    >
                                        {{ entry.issue.team.key.charAt(0) }}
                                    </span>
                                    <PriorityIcon :priority="entry.issue.priority" />
                                    <StatusIcon
                                        :type="entry.issue.state?.type ?? 'unstarted'"
                                        :color="entry.issue.state?.color"
                                    />
                                    <span class="font-mono text-[11px] text-muted-foreground">{{ entry.issue.identifier }}</span>
                                    <span class="min-w-0 truncate text-foreground">{{ entry.issue.title }}</span>
                                </div>
